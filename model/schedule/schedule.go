...
type ScheduleModel struct {
	...
	Group           string `gorm:"column:group_name;type:varchar(20)" json:"group"`
...
func Update(email string, group string) error {
	if err := model.DB.Self.Table("schedules").Where("email=?", email).Updates(map[string]interface{}{"group_name": group, "form_status": 1}).Error; err != nil {
		return err
	}
	return nil
}
...
func Update4(group string) error {
	if err := model.DB.Self.Table("schedules").Where("group_name=?", group).Updates(map[string]interface{}{"work_status": 0}).Error; err != nil {
		return err
	}
	return nil
}
